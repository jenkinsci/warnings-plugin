...
public class XlcCompilerParser extends RegexpLineParser {
    ...
    private static final String XLC_WARNING_PATTERN_WITH_LINE = ANT_TASK
            + "\"?([^\"]*)\"?, line ([0-9]+)\\.[0-9]+:( [0-9]+-[0-9]+)? \\(([USEWI])\\)\\s*(.*)$";
    private static final String XLC_WARNING_PATTERN_NO_LINE = ANT_TASK
            + "\\s*([0-9]+-[0-9]+)?:? \\(([USEWI])\\)( INFORMATION:)?\\s*(.*)$";
